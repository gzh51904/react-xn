...
// let name = req.query.xxx;
console.log(req.query)

query(`select * from ${req.query.name}`).then(function(data){
...
// console.log(id,name);
    ...
    // });
    console.log('klfdghorighowerigh');
    query(`select * from ${name} where id=${id}`).then(function(data){
        // data: promise对象改成resolve状态时传回的参数
        res.send(formatData({data}))
    },(err)=>{
        res.send(formatData({data:err,code:250}))
